"""Load rumi.yaml from disk; run cross-entity validation.
...
from rumi.governance.ast import validate_policy_ast as validate_ast
...
def load(path: Path) -> ValidatedConfig:
    ...
    try:
        config = RumiConfig.model_validate(raw)
...
def _validate_cross_entity(cfg: RumiConfig) -> dict[str, frozenset[str]]:
