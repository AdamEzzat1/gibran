...
in rumi_metric_dependencies in V1)."""
...
class Catalog:
    ...
    def get_source_uri(self, source_id: str) -> str:
        row = self.con.execute(
            "SELECT uri FROM rumi_sources WHERE source_id = ?", [source_id],
        ).fetchone()
        if row is None:
    ...
    def get_dimension(self, dimension_id: str) -> _DimensionMeta:
        row = self.con.execute(
            "SELECT column_name, dim_type FROM rumi_dimensions WHERE dimension_id = ?",
            [dimension_id],
        ).fetchone()
        if row is None:
...
def _render_metric_expression(
    ...
    # rumi_metric_versions.expression for count/sum/avg/min/max/percentile
