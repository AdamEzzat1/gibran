...
The Protocol contract is in rumi.governance.types.GovernanceAPI; this
...
from rumi.observability.types import ObservabilityAPI
...
class DefaultGovernance:
    ...
    def preview_schema(
        ...
        source_row = self.con.execute(
            "SELECT display_name, schema_version FROM rumi_sources WHERE source_id = ?",
            [source_id],
        ).fetchone()
    ...
    def evaluate(
        ...
        for metric_id in requested_metrics:
            row = self.con.execute(
                "SELECT current_version, source_id FROM rumi_metrics WHERE metric_id = ?",
                [metric_id],
            ).fetchone()
    ...
    def _all_columns_for(self, source_id: str) -> set[str]:
        return {
            ...
            for r in self.con.execute(
                "SELECT column_name FROM rumi_columns WHERE source_id = ?",
                [source_id],
            ).fetchall()
        }
    ...
    def _compute_allowed_columns(
        ...
        overrides = dict(
            self.con.execute(
                "SELECT column_name, granted FROM rumi_policy_columns WHERE policy_id = ?",
                [policy_id],
            ).fetchall()
        )
        allowed: set[str] = set()
    ...
    def _build_column_views(
        ...
        rows = self.con.execute(
            ...
            "FROM rumi_columns WHERE source_id = ? ORDER BY column_name",
            [source_id],
        ).fetchall()
        return tuple(
            ColumnView(
    ...
    def _build_dimension_views(
        ...
        rows = self.con.execute(
            ...
            "FROM rumi_dimensions WHERE source_id = ? ORDER BY dimension_id",
            [source_id],
        ).fetchall()
        return tuple(
            DimensionView(
    ...
    def _build_metric_views(self, source_id: str) -> tuple[MetricView, ...]:
        metric_rows = self.con.execute(
            ...
            "FROM rumi_metrics WHERE source_id = ? ORDER BY metric_id",
            [source_id],
        ).fetchall()
        if not metric_rows:
            return ()
        ...
        dep_rows = self.con.execute(
            f"SELECT metric_id, depends_on_id FROM rumi_metric_dependencies "
