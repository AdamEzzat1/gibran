...
- EnvResolver: local development. Reads GIBRAN_DEV_USER / GIBRAN_DEV_ROLE /
  GIBRAN_DEV_ATTRS. Refuses to operate when GIBRAN_ENV != 'dev' to prevent
  accidental prod use.
...
class EnvResolver:
    """Local-dev resolver. Reads GIBRAN_DEV_USER, GIBRAN_DEV_ROLE, GIBRAN_DEV_ATTRS.

    Attributes are encoded as comma-separated key=value pairs:
      GIBRAN_DEV_ATTRS='region=west,partner_id=acme'

    Refuses to operate when GIBRAN_ENV != 'dev' (prevents accidental prod use)."""

    def resolve(self, request_context: object) -> IdentityContext:
        if os.environ.get("GIBRAN_ENV") != "dev":
            raise RuntimeError("EnvResolver requires GIBRAN_ENV=dev")
        user = os.environ.get("GIBRAN_DEV_USER")
        role = os.environ.get("GIBRAN_DEV_ROLE")
        if not user or not role:
            raise RuntimeError(
                "EnvResolver requires GIBRAN_DEV_USER and GIBRAN_DEV_ROLE"
            )
        attrs: dict[str, str] = {}
        raw_attrs = os.environ.get("GIBRAN_DEV_ATTRS", "")
        for pair in raw_attrs.split(","):
            ...
            if "=" not in pair:
                raise RuntimeError(
                    f"GIBRAN_DEV_ATTRS pair must be key=value, got {pair!r}"
                )
            k, v = pair.split("=", 1)
