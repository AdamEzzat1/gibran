"""Audit-log literal redaction for `gibran_query_log`.
...
values from filter predicates:
  ...
  - `nl_prompt`         -- for DSL-path queries, the JSON intent (per
                           gibran_constraints.md the field was repurposed
...
def lookup_sensitive_columns(
    ...
    if source_id is None:
        rows = con.execute(
            "SELECT DISTINCT column_name FROM gibran_columns "
            "WHERE sensitivity IN ('pii', 'restricted')"
        ).fetchall()
    else:
        rows = con.execute(
            "SELECT column_name FROM gibran_columns "
...
def redact_audit_payload(
    ...
    INSERTing into gibran_query_log, ensuring no path bypasses redaction.
    """
