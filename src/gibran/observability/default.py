...
V2 path: latest_blocking_failures reads from gibran_source_health (O(1) PK
lookup) when the row exists. The cache is populated by refresh_health,
which is called by `gibran check` after running rules.

V1.5 fallback: when no gibran_source_health row exists for a source (e.g.
the first time a query touches it, before any `gibran check` has run), we
re-aggregate from gibran_quality_runs + the rule tables directly. This means
...
SELECT
    ...
    CASE WHEN l.ran_at IS NULL
         ...
         ELSE DATE_DIFF('second', l.ran_at, now())
    END AS seconds_since
FROM gibran_freshness_rules fr
...
class DefaultObservability:
    ...
    def latest_blocking_failures(
        ...
        cache_row = self.con.execute(
            "SELECT blocking_failures FROM gibran_source_health WHERE source_id = ?",
            [source_id],
        ).fetchone()
    ...
    # ------------------------------------------------------------------
    # refresh_health: denormalize latest state into gibran_source_health
    # ------------------------------------------------------------------

    def refresh_health(self, source_id: str) -> None:
        """Recompute the cached blocking_failures for a source.

        Idempotent. Called by `gibran check` after running rules. Safe to
        ...
        # gibran_source_health is a leaf table (no inbound FKs); ON CONFLICT
        # DO UPDATE works here (unlike gibran_metrics where the FK quirk bites).
        self.con.execute(
            "INSERT INTO gibran_source_health "
