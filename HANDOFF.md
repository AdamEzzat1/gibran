...
This document is the entry point for whoever picks up Gibran next — whether
...
## 1. What Gibran is, in one paragraph

Gibran is a **governed semantic / metric layer** packaged as a Python
...
| Surface | Works today |
|---|---|
| `gibran init` / `gibran sync` / `gibran check` / `gibran query` (raw SQL + `--dsl`) | All four commands wired end-to-end, exit codes structured for CI. |
...
| Observability | Quality + freshness rules with cache-first reads (`gibran_source_health`), V1.5 SQL-aggregation fallback, `gibran check` runner |
| Audit | Every attempt — allow, deny, error — records a `gibran_query_log` row with deny_reason and (for DSL) the original intent JSON |
...
**Reference**: the relevant rendering goes in `src/gibran/dsl/compile.py:_render_metric_expression`, parallel to the existing `_render_ratio` helper.
...
- **Source-type dispatch in the runner** (S). `gibran check` and the DSL
  ...
  reads `gibran_sources.source_type` and constructs the right FROM clause
  (e.g., `read_parquet('uri')` for parquet) would remove the manual step.
  Real adoption blocker. ~50 lines.
- **Compiled-DSL plan cache** (M). Every `gibran query --dsl '{...}'` re-parses
...
- **Query-plan introspection** (S). Expose DuckDB's EXPLAIN through
  `gibran query --explain` or `gibran explain --dsl '{...}'`. Closes a real
  visibility gap.
...
- **Anomaly-detection rule type** (M). New `quality_rules.rule_type='anomaly'`:
  ...
  mean. Reuses `gibran_query_log` history. Lands as a 6th rule_type.
  Table-stakes for modern data products.
- **`gibran check` scheduler** (M). Today it's manual or cron. An in-product
  scheduler — either a `gibran check --watch` mode or a separate `gibran
...
- **Rule dependencies** (S). "Freshness must pass before quality runs"
  ...
  `gibran_quality_rules` + dependency-aware ordering in `run_checks`.
- **Schema-drift detection in `gibran sync`** (S, high-leverage). When sync
  notices that a source's actual DuckDB schema diverges from `gibran_columns`
...
- **Time-bound policies** (S). `valid_until TIMESTAMP` column on
  `gibran_policies`; `governance.evaluate` checks before allowing.
  Contractor / consultant / temporary access. ~20 lines + migration.
- **Audit-log SQL redaction** (S). `gibran_query_log.generated_sql` includes
...
- **Approval workflow** (M). High-sensitivity policy changes (e.g., any
  ...
  enforced via a `gibran sync --require-approval` mode that writes pending
  changes to a separate table awaiting approval.
- **Access-pattern anomaly detection** (M). "User X just queried 100x
  their typical daily volume" → alert. Reuses `gibran_query_log`.
...
- **`gibran explain --dsl '{...}'`** (S). Given an intent, return what
...
- **Populate `example_values`** (S). `AllowedSchema.ColumnView.example_values`
  ...
  values and store them on `gibran_columns`. Required for NL prompt
  construction and useful for `gibran describe`.
...
To move to A: ship `gibran explain` (V1-week work), then choose one of
...
- **DSL result caching** (M). Same intent + same identity + same
  ...
  `gibran check`. Reduces dashboard load.
- **Compiled-DSL plan cache** (M, also under DB Architect). Avoid
  recompilation on repeated intents.
- **Materialized metrics** (L). `materialized: true` flag → `gibran sync`
...
- **Introspection bundle** (M, high user-visible value):
  - `gibran describe <source>` — show AllowedSchema for the identity:
    columns (with sensitivity), dimensions, metrics, the row filter
    that gets applied to them.
  - `gibran catalog` — list all sources the identity can see.
  - `gibran explain --dsl '{...}'` — parse + validate + compile WITHOUT
    executing.
  - `gibran query --output csv|json|parquet` — result export. Without it,
    Gibran is a CLI demo, not an analyst tool.
- **README + quickstart** (S). A top-level `README.md` aimed at first-time
  visitors. `gibran init --sample` that drops a runnable fixture +
  three-line "now run these three commands" demo.
- **One real user** (no estimate). The grade does not move past B- until
  someone outside the dev environment uses Gibran for a real workflow.
- **Web UI for catalog browsing** (L, V2). The `gibran catalog` data
...
- **Source-type dispatch** (DB Architect's #2) — without it, every new
  ...
  adoption friction.
- **`gibran describe`** (PM's #1) — without it, new users can't discover
  what's available without reading YAML. First-five-minutes UX.
- **Result export** (PM's #2) — without it, Gibran is a CLI demo, not an
  analyst tool.
...
Depth. The dashboard story needs `gibran describe` + result export + maybe
a `gibran init --sample` that drops a runnable fixture. The 5 aggregate
...
changed: (a) `gibran explain` moves into the introspection bundle (small +
...
1. **Source-type dispatch** — closes adoption friction (`gibran check`
   currently breaks on parquet sources unless users CREATE VIEW manually).
2. **Introspection bundle** — `gibran describe`, `gibran catalog`,
   `gibran explain --dsl`, `gibran query --output csv|json|parquet`. Four
   sub-features that ship well together (they share introspection
   plumbing). Without these, Gibran is a CLI demo, not an analyst tool.
3. **`period_over_period` metric primitive** — MoM/QoQ/YoY are the most-asked
   analytics in this space.
4. **README + `gibran init --sample`** — the project doesn't have a
...
6. **Time-bound policies + audit-log SQL redaction** (Sec/Gov A-move).
   *Done at `4b0e8a1` (2026-05-13). `valid_until TIMESTAMP` on gibran_policies
...
src/gibran/
  ...
  sync/
    yaml_schema.py   # Pydantic models for gibran.yaml
...
| Build the wheel | `python -m build --wheel` → `dist/gibran-0.0.1-py3-none-any.whl` |
| Install editable | `pip install -e .` |
| Smoke-test from a clean venv | `python -m venv X; X/Scripts/pip install dist/*.whl; X/Scripts/gibran --help` |
| Run an end-to-end demo | See "Smoke transcripts" in `STATUS.md` |

### How to add a new metric primitive (cookbook)

1. Extend `MetricType` Literal in `src/gibran/sync/yaml_schema.py:MetricConfig`.
...
4. Add the SQL rendering in `src/gibran/sync/applier.py:_render_expression`.
5. If the type's stored expression is self-contained: add it to the
   "use directly" branch in `src/gibran/dsl/compile.py:_render_metric_expression`.
...
7. If it has compatibility constraints (like rolling_window forbidding
   dimensions), add validation in `src/gibran/dsl/validate.py:validate_intent`.
8. Add to the fixture `tests/fixtures/gibran.yaml` and update affected
...
1. Add a `@app.command()` function in `src/gibran/cli/main.py`.
...
3. DuckDB FK enforcement during UPDATE has quirks (see comments in
   migration 0001 above `gibran_metric_versions`). Inbound FKs to a table
...
The Gibran memory files at
`C:\Users\adame\.claude\projects\C--Users-adame-Gibran\memory\` contain
...
1. **Multi-process safety / server mode.** DuckDB is single-writer per
   file. Multi-process Gibran effectively means a server with its own DB
   handle. Decision-deferred until deployment shape is known.
2. **Where does the `gibran check` scheduler run?** Today it's manual or
   cron. A real product needs a scheduler (eventually). Could be a
   sidecar process, a systemd timer, or an in-Gibran scheduler thread.
3. **Rate limiting in a multi-process world.** Per-process token buckets
   ...
   deferred.
4. **Audit log retention + archival.** `gibran_query_log` grows unbounded.
   Need a policy + a `gibran prune` command. Not urgent.
...
FIXED CONSTRAINTS, `memory/gibran_constraints.md`, and §4 ML/NLP role
...
mean?" against `gibran_query_log` (access patterns) or
`gibran_metric_versions` history (metric values). They could share a
`gibran anomaly` runner or be implemented as two `rule_type` variants.
